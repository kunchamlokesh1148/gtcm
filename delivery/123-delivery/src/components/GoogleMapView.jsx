        ...
        const infoWindow = new window.google.maps.InfoWindow({
          content: `
            <div style="color: #0f172a; font-family: sans-serif; padding: 6px; max-width: 220px;">
              <strong style="font-size: 13px; display: block; margin-bottom: 3px; color: #059669;">${title}</strong>
      ...
      <div className={`relative rounded-2xl overflow-hidden border border-white/10 bg-slate-900 shadow-xl ${className}`}>
        ...
        <div className="absolute top-3 left-3 z-10 flex items-center space-x-2">
          <button
            ...
            className="px-2.5 py-1.5 rounded-xl bg-slate-950/90 hover:bg-slate-900 text-xs font-bold text-slate-200 border border-white/10 shadow-lg backdrop-blur-md flex items-center space-x-1.5 transition"
          ...
          <button
            ...
            className={`px-2.5 py-1.5 rounded-xl text-xs font-bold border shadow-lg backdrop-blur-md flex items-center space-x-1.5 transition ${
              showTraffic
                ? 'bg-emerald-600 text-white border-emerald-600'
                : 'bg-slate-950/90 hover:bg-slate-900 text-slate-200 border-white/10'
            }`}
        ...
        {/* Route Details Badge */}
        {routeDetails && (
          <div className="absolute top-3 right-3 z-10 px-3 py-1.5 rounded-xl bg-emerald-950/90 text-emerald-300 border border-emerald-700/60 shadow-lg backdrop-blur-md flex items-center space-x-2 text-xs font-bold">
        ...
        {showExternalButton && (
          <a
            ...
            className="absolute bottom-3 right-3 z-10 px-3 py-1.5 rounded-xl bg-slate-950/90 hover:bg-slate-900 text-xs font-bold text-white border border-white/10 backdrop-blur-md flex items-center space-x-1.5 shadow-lg no-underline transition"
          >
            <Navigation className="w-3.5 h-3.5 text-emerald-400" />
            <span>{hasRoute ? "Start Google Maps Navigation" : "Open in Google Maps"}</span>
      ...
      {showExternalButton && (
        <div className="relative z-10 pt-2 border-t border-white/5 flex justify-end">
          ...
          >
            ...
            <span>Open Driving Route Navigation</span>
